...
describe("Drupal 8 ReST client", () => {
  let mockAxiosHttpClient;

  beforeEach(() => {
    mockAxiosHttpClient = new MockAdapter(axios);
  })

  afterEach(() => {
    mockAxiosHttpClient.reset();
  });

  after(() => {
    mockAxiosHttpClient.restore();
  })

  it('should return the response data in the normal case', (done) => {
    mockAxiosHttpClient.onGet('/node/1/').reply(200, {
      value: 1
    });
  ...
  it('should respond with null if no data is found', (done) => {
    mockAxiosHttpClient.onGet('/node/2/').reply(204);
  ...
  it('should respond with null if the server returns a 404', (done) => {
    mockAxiosHttpClient.onGet('/node/3/').reply(404);

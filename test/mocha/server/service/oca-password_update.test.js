...
describe('LINC Soap Password updater', function() {
  describe('response handling tests', function() {
    ...
    it('should result in error if no lincPasswordUpdate record in DB', function() {
      ...
      findOneSuccessfulStub = sinon
        .stub(lincPasswordUpdate, 'findOne')
        .returns(Promise.resolve(fakePasswordUpdateData))
      getEndPointUrlStub = sinon
        .stub(myObj, 'getEndPointUrl')
        .returns(Promise.resolve('https://catweb2.sba.gov/linc/ws/linc.cfc'))
  ...
  it('should parse the error response properly ', function(done) {
    let expectedResult = {
      ...
      errorMessageTechnical:
        'The stored procedure to update the password did not return the expected response. Your account has been locked. Please call CLS at 571.419.6359 to unlock your account.',
      responseCode: '-15'
    }

...
class RadioButtonGroup extends React.Component {
  ...
  render() {
    ...
    let radioButtons = this.props.options.map(function(item, index) {
        let id = "radio"+index;
        let radioInput = (
          <input className={styles.regularRadio} type="radio" name={me.props.name} checked={item.value === me.props.value}
              onClick={me.handleClick.bind(this)} id={id} value={item.value}></input>
        );
        let radioLabel = (

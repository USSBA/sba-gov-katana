...
class ReadMore extends React.Component{
    ...
    makeExpanded(text) {
      if (text) {
          ...
          return split.map(function(item, index){
              return (
                  <p key={index} className={styles.expandedCopyText}>{item} </p>
              );
          });
      } else {
        return (<div/>);
    ...
    render(){
        ...
        return (<div className={styles.readMoreSection}>
            ...
            <p key={50} id={this.props.parentId + "-preview"} className={styles.preview}>{this.props.readMoreSectionItem.preview}</p>
            {expandedHr}
            {expandedTextSection}

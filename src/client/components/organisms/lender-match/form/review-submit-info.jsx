  ...
  render() {
    ...
    return (
      <div id="lender-match-review-and-submit-panel">
        ...
        <form id={formId} ref={(input) => this.reviewSubmitInfoForm = input} onSubmit={(e) => this.handleSubmit(e)}>
          <FormPageButtons parentId={formId} continueButtonFullWidth continueButtonText="SUBMIT" showBackButton={false} continueButtonHandler={this.handleSubmit.bind(this)}/>
        </form>
      </div>
    );
  };
...
const EditButton = (props) => {
  return (<SmallGreySecondaryButton id={props.parentId + "-edit-button"} extraClassName={styles.editButton} autoFocus={props.autofocus} onClick={props.onEditClick} text="EDIT"/>);
};
...
const IndustrySection = (props) => {
  ...
  return (
    <div className={ styles.sectionContainer } id={id}> <h1 className={styles.title}>Industry</h1>
    <p className={styles.field}>
...
function mapStateToProps(state) {
  return {loanData: state.lenderMatch.loanData, additionalInfoData: state.lenderMatch.additionalInfoData, contactInfoData: state.lenderMatch.contactInfoData, businessInfoData: state.lenderMatch.businessInfoData, industryInfoData: state.lenderMatch.industryInfoData};
}

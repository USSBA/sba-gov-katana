...
function fetchUserZipCoords(zipcode) {
  ...
  const query = mysql.format("SELECT * from zipcodes WHERE zipcodes.zip = ?;", zipcode);
...
function createCounselorsByLocationQuery(zipCoords) {
  ...
  const queryArr = [
    ...
    "(COALESCE(ACOS( SIN(39.003809*PI()/180) * SIN(location.latitude*PI()/180) ",
    "+ COS(39.003809*PI()/180) * COS(location.latitude*PI()/180) ",
    "* COS(location.longitude*PI()/180 - -77.30165*PI()/180)), 0.00000) ",
...
function fetchCounselorsByLocation(zipcode) {
  fetchUserZipCoords(zipcode)
    .then(function(zipCoords) {
      const query = createCounselorsByLocationQuery(zipCoords);
      return executeQuery(query)
        .then(function(counselors) {
          createCounselorsByLocationQuery();

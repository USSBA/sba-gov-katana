...
function fetchUserZipCoords(zipcode) {
  ...
  return executeDrupalSevenQuery(query).then(function(zipCoords) {
    return {
...
function fetchCounselorsByLocation(zipcode) {
  ...
  const counselorData = counselorQuery.then(function(query) {
    return executeDrupalSevenQuery(query)
  })
  ...
  return Promise.join(userZipCoords, counselorQuery, counselorData, counselorJson, function(
    zipCoords,
    query,
    counselors,
    counselorsJson
